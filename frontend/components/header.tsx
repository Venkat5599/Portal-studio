    ...
    >
      <div className="h-20 max-[850px]:h-18 flex items-center justify-between px-4 max-[850px]:px-6">
        <a href="#" className="flex items-center gap-2 ml-4 max-[850px]:ml-0">
          ...
          <span className="text-lg font-semibold text-foreground leading-0 max-[1200px]:hidden max-[850px]:inline">PalletMan</span>
        </a>
        ...
        <nav className="flex items-center gap-1 max-[1200px]:gap-0 max-[850px]:hidden">
          ...
          <DesktopDropdown
            ...
            onClose={() => setActiveMenu(null)}
          />
          <a href="#tracks" className="px-4 py-2 max-[1200px]:px-3 text-sm font-medium text-foreground/80 hover:text-foreground transition-colors rounded-full hover:bg-foreground/5">
            Hackathon
          </a>
        </nav>

        <div className="flex items-center gap-4 max-[850px]:hidden">
          <a href="/app" className="text-sm font-medium text-foreground/80 hover:text-foreground transition-colors">
            Docs
          </a>
          <a href="/app" className="group relative inline-flex items-center">
            <span className="absolute right-0 inset-y-0 w-[calc(100%-1.5rem)] rounded-xl bg-accent" />
            <span className="relative z-10 px-5 py-3 rounded-xl bg-foreground text-background text-sm font-medium">Launch studio</span>
          ...
          >
            <div className="px-6 pb-4">
              <nav className="space-y-0">
                <a href="/app" className="flex items-center justify-between py-4 text-base font-medium text-foreground border-b border-foreground/10" onClick={closeMobile}>
                  Demo
                </a>
                <MobileExpandable
                  label="Products"
              ...
              <div className="flex items-center justify-between pt-8 pb-2">
                <a href="/app" className="text-base font-medium text-foreground" onClick={closeMobile}>
                  Docs
                </a>
                <a href="/app" className="group relative inline-flex items-center" onClick={closeMobile}>
                  <span className="absolute right-0 inset-y-0 w-[calc(100%-1.5rem)] rounded-2xl bg-accent" />
                  <span className="relative z-10 px-5 py-3 rounded-2xl bg-foreground text-background text-sm font-medium">Launch studio</span>
